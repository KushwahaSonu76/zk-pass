...
exports.initialLedgerState = initialLedgerState;
...
class Contract {
  constructor(witnesses) {
    ...
    this.circuits = {
      computeCommitment: circuits_1.pureCircuits.computeCommitment,
      calculateMerkleRoot: circuits_1.pureCircuits.calculateMerkleRoot,
      verifyCredentialProof: (context, accessContextHash) => {
        ...
        const commitment = circuits_1.pureCircuits.computeCommitment(secret, salt);
        const computedRoot = circuits_1.pureCircuits.calculateMerkleRoot(commitment, path, index);

        if (computedRoot !== context.ledger.credentialRoot) {
          throw new Error('ZkPass: Invalid credential commitment membership proof');
        }

        const revocationCheck = circuits_1.pureCircuits.calculateMerkleRoot(commitment, path, index);
...
exports.pureCircuits = circuits_1.pureCircuits;
